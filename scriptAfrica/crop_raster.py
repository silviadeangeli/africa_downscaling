...
def downscale_crop (sFileMapSpamLowResTot,sFileMask, sFileClipper):
    ...
    log_print("Total original production value for crop type="+sCropType+": %.2f" % np.nansum(data_low))
    ...
    log_print ("MAPSPAM production value not overlapping mask: %.2f (%.1f %%)" % (excluded_exposure , excluded_exposure/total_exposure*100))
...
sFileMask = '/Users/lauro/Downloads/crop/sz_crops_mask.tif'
sFileBoundary = '/Users/lauro/Downloads/crop/SZ_bou_GAUL_admin0_20180626.shp'

# world crop production directory
dirIn = '/Users/lauro/Downloads/crop'

for file in os.listdir(dirIn):
    if  file.startswith('spam') and file.endswith('tiff'):
        downscale_crop(os.path.join(dirIn,file),sFileMask, sFileBoundary)
